...
One rule about weight falls out of this and is binding beyond the four states: **a filled primary button is reserved for the one action a screen exists to perform.** An empty state's action is the ghost preview's last row, a dead end's way out is outlined, and a retry is the error row's borderless action — because the heaviest ink in the system must never mark a screen that is failing, empty, or waiting. `design-system.md` § buttons carries the full list.\
...
**Date:** 2026-08-07\
**Status:** accepted\
**Context:** ADR-052 ruled that `area` entities "stay links in the sentence and never take a rail slot: they are context for a sentence, not somewhere to go." That was true of the world it was written in — an area link had nowhere to land. kebi ADR-153 changed the world: areas got a row, a profiler, and `GET /v1/areas/{id}`, so every level of the geo hierarchy now opens. Left alone, the client would render an area as an underlined word that does nothing, which is worse than styling it as prose. Screen and chip options were drawn against the real payload and reviewed (`docs/kebi-app-design-system/kebi-area-mockup.html`, `kebi-area-country-mockup.html`, `kebi-chat-area-chip-options.html`).\
**Decision:** Areas take a rail slot, drawn with the **venue chip unchanged** — the emoji is the whole distinction. A separate shape (outline, pill, subtitle) was rejected: it would say "not a venue" and "less than a venue" in the same stroke, and the area is the turn's scope, not a lesser member of it. Ordering stays first-mention with kinds mixed, because an answer names its scope before the places inside it and hoisting would contradict the sentence the rail indexes. The screen behind the tap is **read-only**: breadcrumb, emoji + name, level, saved count, the summary as an uncollapsed hero, best-for chips, and one body section. No save (region interest keeps flowing through kebi's signals), no distance, no ask bar, no taste-ranked suggestions, and no sibling links — discovery stays in chat. The body section swaps contents by scope rather than layout: above the leaf its rows are child areas carrying the caller's save count under that key, at the leaf they are venues, so drilling down is one gesture until it stops.\
**Consequences:** Both entity kinds now behave identically in chat, and the client's link handler dispatches on kind instead of filtering to venues. Two costs are accepted deliberately. The chip's shape says nothing about opening a list rather than a place card, so the *screen* has to establish that on arrival — that is the job of the breadcrumb, the level chip, and rows that each carry a chevron. And with siblings dropped, navigation is breadcrumb-up or row-down only: Canggu to Seminyak goes through Bali. An area with no saves and no notable children renders as profile alone; a country-scope screen cannot surface a region the user has never visited, which is the discovery gap the ask bar would have closed and is the first thing to revisit if the screen reads as a dead end.
...
**Decision:** One authenticated `POST /api/v1/feedback` on the gateway stamps the verified user id and the token-claim email (ADR-044/045 — no DB lookup) and creates a page in a Notion database via the Notion API; no gateway DB writes, no kebi involvement, and no AI gating (ADR-022 — bug reports must flow when AI is killed). The endpoint always returns 202: missing Notion credentials or a Notion failure is logged, never surfaced. Only `wrong_answer` reports attach conversation context, and it is deliberately lean — turn text and reasoning step titles, never tool payloads — matching the in-app disclosure shown before sending (tool names left the attachment with ADR-052: the client no longer sees which tools ran). A small per-user hourly cap guards the fan-out. The client entry points are the chat top bar's `?` (always visible — help doesn't hide; chat is an overlay, so it closes before `/help` pushes) and a settings help row; home stays entry-free (top-pill cap).\
...
## ADR-052: A chat answer is linked prose plus a venue rail — the place card leaves chat

**Date:** 2026-08-07\
**Status:** accepted\
**Context:** kebi stopped sending tool payloads to chat (kebi ADR-136): a turn now returns `message` prose with entity names already wrapped as `kebi://{kind}/{key}` markdown links, a flat `entities` list resolving those links (`kind`, `key`, `name`, `uri`, and an `icon` glyph — kebi ADR-117/146), and the turn's `recommendation_id`. There is no `tool_results` field and no `tool_result` SSE frame, so the client has no candidate list, no per-place metadata, and nothing to build a place card from. That removed ADR-050's subject matter entirely and left chat rendering the whole answer as one text node — real answers stack five paragraphs and a five-item list, so they arrived as a wall of text with literal `- ` markers, and entity links were styled identically to `**bold**`, leaving nothing on screen that read as openable. Design options were drawn against real payloads and reviewed (`docs/kebi-app-design-system/kebi-chat-answer-options.html`).\
**Decision:** The answer is **linked prose plus a rail of the venues it named** (option c). Prose is rendered at block level — paragraphs and bullet items with a hanging indent — and inline an entity link is **underlined and tappable** while bold is neither; that distinction is the affordance, so emphasis is never mistaken for a place. Beneath the prose, one chip per distinct `venue` entity in mention order, carrying kebi's `icon` with a client-side per-kind fallback when it is `null`. `area` entities stay links in the sentence and never take a rail slot: they are context for a sentence, not somewhere to go (**superseded by ADR-053** — areas became openable and now take a slot). Both the links and the chips open that venue's card by resolving it against the caller's library, the only endpoint that returns a renderable place; a venue that does not resolve says so rather than opening a card assembled from a name. Rendering keys on nothing tool-shaped, so a new kebi tool changes what the agent says and never what the client draws.\
**Consequences:** Chat gains no new failure mode when kebi adds a tool, and the client stops carrying a tool vocabulary. The accept/reject/save actions that hung off the old card are gone from chat — `recommendation_id` still arrives, so they can return on the venue card once it is reachable for any place. Until kebi exposes a place-by-id read, a tap only resolves venues the user has already saved, and the lookup walks library pages under a page cap; that lookup is the single seam to replace when the endpoint lands. Web keeps its own thread rendering and did not adopt the rail — its dead consult surfaces were removed rather than ported.
...
**Date:** 2026-07-16\
**Status:** superseded by ADR-052 — kebi ADR-136 took tool payloads off the wire, so there is no `tool_result` frame to discriminate and no candidate list to build a card from.\
